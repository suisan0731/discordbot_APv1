...
TOKEN = "MTE5NDY3NTIxMDcyMTQ0ODA5Nw.GnYzjG.Bop-1FdqHsP22VmyTvCthtr5zRjfV98u5i9nUA"
...
async def join(ctx):
    if ctx.author.voice is None:
        ...
        await ctx.channel.send('```すでに {} チャンネルに参加しています```'.format(ctx.author.voice.channel.name))
        return
...
@bot.command(
        name='play',
        brief='$play <YouTbe URL> で音楽を再生する'
)
